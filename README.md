...
🎥 **کانال یوتیوب رسمی پروژه:**  
https://www.youtube.com/@aliznet  
...
هر بخش قوانین دقیق دارد که در ادامه توضیح داده شده‌اند.
...
6. Pull Request ارسال کنید و توضیح دقیق دهید:
...
## 🟡 ۲.۳. مقداردهی frontmatter (الزامی)
...
✔ `slug` باید با ساختار فولدر شما *همخوانی کامل* داشته باشد.
✔ آدرس نهایی صفحه:
...
✔ `slug` باید شامل `category/` + *نام فولدر* باشد.
✔ `position` ترتیب دسته در سایدبار را مشخص می‌کند.

---

## 🟩 ۲.۵. افزودن دسته به صفحه اصلی سایت
...
alireza:
  name: علیرضا فدایی
  title: توسعه‌دهنده و نویسنده محتوا
  url: https://github.com/alireza
...
## 🔖 ۳.۳. frontmatter الزامی برای بلاگ:
...
✔ `authors` باید لیست شناسه‌های موجود در `authors.yaml` باشد.
✔ `tags` باید از `tags.yaml` انتخاب شده باشد.
...
اگر پیشنهاد یا ایرادی دارید، لطفاً Issue باز کنید.
با هم یک دنیای آموزشی بهتر برای فارسی‌زبانان می‌سازیم. ❤️
